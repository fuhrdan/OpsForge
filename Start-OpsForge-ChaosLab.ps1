...
function Send-LabHeartbeats([bool]$failed) {
  foreach ($node in $nodes) {
    # Three related failures per affected node. Healthy nodes and an unrelated memory warning remain independent.
...
Write-Host "Injected $Scenario for $Agents synthetic agents ($runId). Three node outages and one separate memory warning." -ForegroundColor Yellow
